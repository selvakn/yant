...
func hasRequiredTemplates(dir string) bool {
	required := []string{
		...
		filepath.Join("shared", "history.html"),
...
func createStubTemplateDir(t *testing.T) string {
	...
	stubs := map[string]string{
		...
		filepath.Join("notes", "reader.html"):                   `{{define "content"}}reader:{{.BodyHTML}}{{if .LastEditor}}|last-editor:{{.LastEditor}}{{end}}{{end}}`,
		...
		filepath.Join("shared", "reader.html"):                  `{{define "content"}}shared-reader:{{.Note.Title}}|{{.OwnerUsername}}|role:{{.Role}}|canEdit:{{.CanEdit}}{{if .LastEditor}}|last-editor:{{.LastEditor}}{{end}}{{.BodyHTML}}{{end}}`,
